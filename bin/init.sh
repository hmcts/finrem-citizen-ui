...
declare -a files_with_port=(Dockerfile README.md src/main/server.ts docker-compose.yml charts/finrem-citizen-ui/values.yaml src/test/config.ts src/test/smoke/smoke.ts)
declare -a files_with_product=(package.json Jenkinsfile_CNP.disabled Jenkinsfile_nightly docker-compose.yml src/main/modules/properties-volume/index.ts src/main/modules/appinsights/index.ts charts/finrem-citizen-ui/Chart.yaml charts/finrem-citizen-ui/values.yaml Dockerfile README.md sonar-project.properties)
...
git mv charts/finrem-citizen-ui/ charts/${product}-${component}
