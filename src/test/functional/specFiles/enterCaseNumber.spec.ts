...
test.describe('Enter Case Number Page Verification', () => {
  ...
  for (const { desc, value } of happyPaths) {
    test.skip(`Happy Path: ${desc} @PR @a11y`, async ({ page, enterCaseNumberPage, axeUtils }) => {
  ...
  test('Error: Boundary check - 15 characters @PR @a11y', async ({ enterCaseNumberPage, axeUtils }) => {
  ...
  test('Error: Boundary check - 21 characters @PR @a11y', async ({ enterCaseNumberPage, axeUtils }) => {
