...
test.describe('Enter Case Number Page Verification', () => {
  ...
  for (const { desc, value } of happyPaths) {
    test.skip(`Happy Path: ${desc} @PR`, async ({ page, enterCaseNumberPage, axeUtils }) => {
  ...
  test('Success Logic: 20 digits (Upper Boundary) @PR', async ({ enterCaseNumberPage, axeUtils }) => {
  ...
  test('Error: Empty input @PR', async ({ enterCaseNumberPage, axeUtils }) => {
    await enterCaseNumberPage.submitCaseNumber('');
    await enterCaseNumberPage.expectValidationError('Enter your case number');
    await axeUtils.audit();
  });

  test('Error: Boundary check - 15 characters @PR', async ({ enterCaseNumberPage, axeUtils }) => {
  ...
  test('Error: Boundary check - 21 characters @PR', async ({ enterCaseNumberPage, axeUtils }) => {
    await enterCaseNumberPage.submitCaseNumber(dataFactory.generateDigits(21));
    await enterCaseNumberPage.expectValidationError('Case number must be between 16 and 20 characters');
    await axeUtils.audit();
  });

  test('Error: Invalid format - Letters @PR', async ({ enterCaseNumberPage, axeUtils }) => {
  ...
  test('Error: Case number not found @PR', async ({ enterCaseNumberPage, axeUtils }) => {
